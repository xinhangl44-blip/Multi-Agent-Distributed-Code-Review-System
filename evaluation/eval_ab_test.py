...
def load_hardware_aligned_model(with_adapter=False):
    ...
    if with_adapter:
        ...
        print("Successfully mounted local fine-tuned model with LoRA weights.")
    else:
        model = base
        print("Loaded pure base model (without LoRA).")
    model.eval()
...
if __name__ == "__main__":
    print("\n[Phase 1/2] Benchmarking original base model...")
    ...
    print("\n[Phase 2/2] Benchmarking local fine-tuned model...")
    ...
    print("\n" + "="*60)
    print("[Week 8 Core Deliverable: 20-Case Complex Prompt A/B Evaluation Report]")
    print("="*60)
    print(f"Base Model        Accuracy: {b_acc:.1f}% ({b_correct}/{total}) | Parse Errors: {b_parse_err}")
    print(f"Fine-tuned Model Accuracy: {ft_acc:.1f}% ({ft_correct}/{total}) | Parse Errors: {ft_parse_err}")
    print(f"Net Accuracy Gain (Delta Net Gain): +{ft_acc - b_acc:.1f}%")
    print("="*60)
    print("\n🔍 [Diagnostic Comparison: First 3 Test Cases Raw Output]")
    for i in range(3):
        print(f"\n[Case #{b_res[i]['idx']}] Expected Decision: {b_res[i]['expected']}")
        print(f"   Base Raw Output       -> Decision: [{b_res[i]['decision']}] | Output: {b_res[i]['raw'][:100]}...")
        print(f"   Fine-tuned Raw Output -> Decision: [{ft_res[i]['decision']}] | Output: {ft_res[i]['raw'][:100]}...")
