...
class KafkaEventBus:
    def __init__(self):
        logger.info("🔌 [Kafka Bus] 正在建立 K8s 远端 Kafka 骨干网连接...")
    ...
    def get_consumer(self, topic: str, timeout_ms: int = 5000) -> KafkaConsumer:
        ...
        return KafkaConsumer(
            ...
            bootstrap_servers=KAFKA_BOOTSTRAP,
            group_id=group_id,
            auto_offset_reset='earliest',  # 🌟 核心修复：改回 earliest，确保上线立刻消费刚才 submit 的消息
    ...
    def publish(self, topic: str, message: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"📤 [Kafka Pub] ──> Topic: {topic.ljust(25)} | Event: {message['payload']['status']}", extra={"request_id": req_id})
...
class BaseAgentWorker:
    ...
    def consume_and_process(self):
        for msg_block in self.consumer:
            ...
            try:
                ...
                logger.error(f"❌ [Worker 异常] {self.__class__.__name__} 失败: {str(e)}", extra={"request_id": req_id})
                if meta["retry_count"] < self.max_retries:
                    meta["retry_count"] += 1
                    msg["metadata"] = meta
                    logger.warning(f"🔄 [Retry Flow] 正在向重试队列投递. Count: {meta['retry_count']}", extra={"request_id": req_id})
    ...
    def call_vllm_with_limit(self, role: str, messages: List[Dict[str, str]], max_tokens: int, req_id: str) -> str:
        ...
        logger.info(f"⏱️  [vLLM Metrics] 角色: {role.ljust(8)} | 耗时: {duration:.4f}s", extra={"request_id": req_id})
        return response.choices[0].message.content

# ==========================================
# 5. 极致防御型 Agent Worker 逻辑
# ==========================================
class PlannerWorker(BaseAgentWorker):
    def business_logic(self, payload: Dict[str, Any], req_id: str, meta: Dict[str, Any]) -> Dict[str, Any]:
        logger.info("🧠 [PlannerWorker] 发起微型故障拆解...", extra={"request_id": req_id})
        # 🟢 系统提示词极致瘦身 + Bug 截断 200 字符
        messages = [
...
class RetrieverWorker(BaseAgentWorker):
    def business_logic(self, payload: Dict[str, Any], req_id: str, meta: Dict[str, Any]) -> Dict[str, Any]:
        logger.info("🔍 [RetrieverWorker] 正在向 RAG 检索截断上下文...", extra={"request_id": req_id})
        
        if "division by zero" in payload["data"]["bug_report"].lower() and meta["retry_count"] == 0:
            raise ConnectionError("Qdrant gRPC Cluster connection timeout on 6334 port!")
        ...
        if not hits:
            raise RuntimeError("RAG 未匹配到任何源码")
...
class CoderWorker(BaseAgentWorker):
    def business_logic(self, payload: Dict[str, Any], req_id: str, meta: Dict[str, Any]) -> Dict[str, Any]:
        logger.info("💻 [CoderWorker] 触发高容错 Patch 推理...", extra={"request_id": req_id})
...
class VerifierWorker(BaseAgentWorker):
    def business_logic(self, payload: Dict[str, Any], req_id: str, meta: Dict[str, Any]) -> Dict[str, Any]:
        logger.info("🧪 [VerifierWorker] 触发微型门禁审核...", extra={"request_id": req_id})
        
        # 🟢 Verifier 接收截断后的 patch 描述
        full_user_content = (
            f"{payload['data']['shared_context']}\n\n"
        ...
        # 🟢 max_tokens 收缩至 32 (只需要输出 PASS/FAIL 级短句)
...
if __name__ == "__main__":
    ...
    # 弹性重试扫描驱动
    logger.info("🔄 [Orchestrator] 扫描弹性重试队列（Retriever.retry）...")
    ...
    kafka_bus.flush()
    kafka_bus.close()
    print("👋 [System] 数据通道安全释放，全量通关演练圆满收官。")
